...
src/
  ...
  edit.rs              Hash-anchored editing (srcwalk_edit). Hashline verification + atomic apply.
  install.rs           `srcwalk install <host>` — writes MCP config for 6 hosts.
...
cargo install --path .       # install to ~/.cargo/bin/srcwalk
```
...
```bash
# Full suite: all tasks, baseline + srcwalk, 3 reps per task
python benchmark/run.py --models sonnet --reps 3 --tasks all --modes all

# Specific tasks
python benchmark/run.py --models haiku --reps 3 --tasks rg_search_dispatch,rg_trait_implementors --modes srcwalk

# Models: sonnet, opus, haiku, gpt5, o3
# Modes: baseline (built-in tools), srcwalk (built-in + srcwalk MCP), srcwalk_forced (srcwalk MCP only)
...
jq -r '[.task, (.correct|tostring), (.total_cost_usd|tostring), (.tool_calls.srcwalk_search // 0 | tostring)] | join("\t")' benchmark/results/<file>.jsonl
```

Results written to `benchmark/results/benchmark_<timestamp>_<model>.jsonl`. Each line is JSON with: `task`, `mode`, `model`, `correct`, `total_cost_usd`, `num_turns`, `tool_calls` (map of tool name → count), `tool_sequence`, `srcwalk_version`, `duration_ms`, token counts.
