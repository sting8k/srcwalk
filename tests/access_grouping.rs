...
#[test]
fn access_default_result_page_is_bounded() {
    let dir = temp_repo("access_default_limit");
    let mut body = String::new();
    body.push_str("typedef struct engine_s engine_t;\n\n");
    for idx in 0..60 {
        body.push_str(&format!(
            "void mark_{idx}(engine_t *e) {{\n    e->is_args = {idx};\n}}\n\n"
        ));
    }
    write_file(&dir.join("src/many.c"), &body);
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("hits: total=60 shown=50 write=59 reset=1 read=0 unknown=0"),
        "default page should show a bounded first page while preserving totals:\n{stdout}"
    );
    let detailed_listing = stdout
        .split("\n\n## src/many.c")
        .nth(1)
        .and_then(|tail| tail.split("\n\n(~").next())
        .unwrap_or_default();
    assert_eq!(
        detailed_listing.matches("\n- :").count(),
        50,
        "detailed access rows should default-cap at 50:\n{stdout}"
    );
    assert!(
        !stdout.contains("mark_59 write="),
        "default function summary should not format functions outside the first page:\n{stdout}"
    );
    assert!(
        stdout.contains("--offset 50"),
        "footer should point to the next page when the default cap applies:\n{stdout}"
    );
