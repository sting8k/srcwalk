...
/// A matched alias becomes local without reclassifying the package import.
/// The marker is emitted only for the alias-derived local target.
#[test]
fn alias_local_and_package_external_counts_remain_distinct() {
    let root = fixture("alias-counts");
    write(
        &root,
        "tsconfig.json",
        r#"{"compilerOptions":{"paths":{"@lib/*":["src/lib/*"]}}}"#,
    );
    write(&root, "src/lib/util.ts", "export const util = 1;\n");
    write(&root, "rel.ts", "export const rel = 1;\n");
    write(
        &root,
        "main.ts",
    ...
    let (ok, stdout, stderr) = deps_output(&root, "main.ts", &[]);
    assert!(ok, "stdout:\n{stdout}\nstderr:\n{stderr}");
    assert_eq!(
        stdout.lines().next().expect("header line"),
        "# Deps: main.ts — 2 local, 1 external, 0 dependents",
        "{stdout}"
    );
    assert!(stdout.contains("## Uses (external)\ncrypto"), "{stdout}");
    assert!(
        stdout.contains("  util.ts (via tsconfig paths)"),
        "{stdout}"
    );
    assert_eq!(
        stdout.matches("(via tsconfig paths)").count(),
        1,
        "{stdout}"
    );
